...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        ...
        const { name, dob, gender, email, phone, address, 
                height, weight, heartRate, bodyFat, 
                goalType, targetValue } = req.body;
...
app.post('/api/member/book-pt', async (req, res) => {
    ...
    try {
        const check = await pool.query(
            'SELECT * FROM PTSession WHERE trainer_id = $1 AND session_time = $2',
            [trainerId, time]
        );
        if (check.rows.length > 0) {
            return res.json({ success: false, message: 'Trainer is already booked at this time.' });
        }

        await pool.query(
            'INSERT INTO PTSession (member_id, trainer_id, room_id, session_time, status) VALUES ($1, $2, $3, $4, $5)',
            [memberId, trainerId, roomId, time, 'scheduled']
        );
...
app.post('/api/trainer/availability', async (req, res) => {
    ...
    try {
        await pool.query(
            'INSERT INTO TrainerAvailability (trainer_id, start_time, end_time) VALUES ($1, $2, $3)',
...
app.post('/api/admin/maintenance', async (req, res) => {
    ...
    try {
        await pool.query(
            'INSERT INTO MaintenanceLog (equipment_id, issue_description, repair_status) VALUES ($1, $2, $3)',
            [equipmentId, description, 'pending']
        );
